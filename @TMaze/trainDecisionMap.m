function obj = trainDecisionMap(obj, options)
...
trialIdx = 1:length(obj.contrastSequence);
nTrials = length(trialIdx);
[thAll, zAll, ~] = buildVectors(obj, trialIdx);
...
[optStd, errVals, errValMatrix, stdGridValues] = ...
        estOptimalStd(coords, dVector, {zEdges, thEdges}, options.errPower);

obj.trainingData.optStd = optStd;
obj.trainingData.errVals = errVals;
obj.trainingData.errValMatrix = errValMatrix;
obj.trainingData.stdGridValues = stdGridValues;
obj.trainingData.zEdges = zEdges;
obj.trainingData.thEdges = thEdges;

hFilter = ndGaussian(optStd);
[theMap, binCentres] = estMap(cell2mat(coords), cell2mat(dVector), {zEdges, thEdges}, hFilter);
hFilter = ndGaussian([0.2 0.2]);
[rawMap, ~] = estMap(cell2mat(coords), cell2mat(dVector), {zEdges, thEdges}, hFilter);
...
obj.trainingData.options = options;
obj.trainingData.decisionMap = theMap;
obj.trainingData.decisionBinCentres = binCentres;
obj.trainingData.rawDataMap = rawMap;

end % TrainDecisionMap()
