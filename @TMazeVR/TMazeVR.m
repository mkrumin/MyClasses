classdef TMazeVR
    ...
    methods
        function obj = TMazeVR(expRef)
            fprintf('Starting the TMazeVR constructor for experiment %s...\n', expRef);
            obj.expRef = expRef;
            fprintf('Loading and processing behavior data...\n');
            obj.dataTMaze = TMaze(expRef);
            obj.dataTMaze = getPCData(obj.dataTMaze);
            obj.dataTMaze = fitPC(obj.dataTMaze);
            ...
            fprintf('Getting reward times...\n');
            rewardIdx = find(ismember({obj.dataTL.hw.inputs.name}, 'rewardCommand'));
            rewardSignal = obj.dataTL.rawDAQData(:, rewardIdx);
            rewardSignal = rewardSignal/5; % 'normalizing' between 0 and 1
            below = rewardSignal<0.5;
            obj.timesReward = obj.dataTL.rawDAQTimestamps(find(diff(below)==1)+1);
            
            fprintf('Getting sounds'' times...\n');
            soundIdx = find(ismember({obj.dataTL.hw.inputs.name}, 'audioMonitor'));
            soundSignal = obj.dataTL.rawDAQData(:, soundIdx);
            % N = 50;
            % b = ones(N, 1)/N;
            % fSoundSignal = filtfilt(b, 1, soundSignal.^2);
            %
            fSoundSignal = soundSignal.^2;
            fSoundSignal = fSoundSignal/max(fSoundSignal);
            above = fSoundSignal > 0.05;
            goingUp = find([0; diff(above)]==1);
            idx = goingUp(find(diff([1; goingUp])*obj.dataTL.hw.samplingInterval > 0.2));
            obj.timesSoundStart = obj.dataTL.rawDAQTimestamps(idx);
            
            obj.trainingData = cell(obj.info.nPlanes, 1);
            fprintf('\nYay! TMazeVR constructor fininshed working! \n');
