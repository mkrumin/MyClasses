function [fMatrixModel] = estTracesFromMap(obj, iPlane, iROI, nPoints, options)
...
optStd = obj.trainingData{iPlane}(iROI).optStd(end,:);
hGauss = ndGaussian(optStd);
[theMap, binCentres] = estMap([zAll, thAll], fAll, {zEdges, thEdges}, hGauss);


%% now let's estimate the best scaling factor for each contrast
...
for iC = 1:nContrastGroups
    for iD = 1:nDecisionGroups
        ...
        fMap = interp2(X, Y, theMap, thVector{iD, iC}, zVector{iD, iC}, 'linear');
        fReal = fVector{iD, iC};%median(fAll);
...
[thMatrix, zMatrix, fMatrix] = buildTraces(obj, 1:length(obj.dataTMaze.contrastSequence), tData, fData, nPoints);
fMatrixModel = nan(size(fMatrix));
...
%     tNew = linspace(tValues(1), tValues(end), nPoints);
%     thValues = interp1(tValues, thValues, tNew, 'linear');
%     zValues = interp1(tValues, zValues, tNew, 'linear');
...
% caxis([minF, maxF]);
% subplot(1, 2, 2)
% imagesc(fMatrixModel);
% caxis([minF, maxF]);
% colormap jet
% 
% figure
% hold off
% plot(mean(fMatrix, 2), 'b')
% hold on;
% plot(mean(fMatrixModel,2), 'r')
...
%     end
%     fMeanData = cat(1, fMeanData, nanmean(fRealL, 2));
%     fMeanScaled = cat(1, fMeanScaled, nanmean(fEstL, 2));
%     fMeanUnscaled = cat(1, fMeanUnscaled, nanmean(fMap, 2));
%     
%     trialIdx = intersect(find(ismember(obj.dataTMaze.contrastSequence, options.contrastGroups{iC})), ...
...
%     end
%     fMeanData = cat(1, fMeanData, nanmean(fRealR, 2));
%     fMeanScaled = cat(1, fMeanScaled, nanmean(fEstR, 2));
%     fMeanUnscaled = cat(1, fMeanUnscaled, nanmean(fMap, 2));
%     
%     nTrials = size(fRealR, 1) + size(fRealL, 1);
%     nanLineThickness = 30;
%     nanLine = nan(nanLineThickness, size(thMatrix, 2));
% %     nanLine = nan(ceil(nTrials/20), size(thMatrix, 2));
%     
%     imHeight = 500;
%     heightL = round(size(fRealL, 1)/(size(fRealL, 1)+size(fRealR, 1))*imHeight);
%     heightR = imHeight-heightL;
%     if heightL>0
%     fRealL = imresize(fRealL, [heightL, size(fRealL, 2)], 'nearest');
%     fEstL = imresize(fEstL, [heightL, size(fEstL, 2)], 'nearest');
%     end
%     if heightR>0
%     fRealR = imresize(fRealR, [heightR, size(fRealR, 2)], 'nearest');
%     fEstR = imresize(fEstR, [heightR, size(fEstR, 2)], 'nearest');
%     end
...
%     cmap = colormap;
%     
%     fReal = setNanColor(fReal, cmap, [0 0 0]);
%     fEst = setNanColor(fEst, cmap, [0 0 0]);
%     
...
% plot(thMatrix', zMatrix', 'k', 'LineWidth', 0.25);
...
% idx = ~isnan(fMeanScaled);
% plot(fMeanData, fMeanScaled, 'b.', fMeanData, fMeanUnscaled, 'r.')
% % plot(fData, fDataModel, '.b', fData, fGlobalModel, '.r');
% hold on;
% axis equal
% % xlim([min([fData(idx); fDataModel(idx)]), max([fData(idx); fDataModel(idx)])]);
% xlim([min([fMeanData(idx); fMeanScaled(idx); fMeanUnscaled(idx)]), max([fMeanData(idx); fMeanScaled(idx); fMeanUnscaled(idx)])]);
% ylim(xlim);
% box off
% plot(xlim, ylim, 'k', 'LineWidth', 2);
% % tmp = corrcoef(fData(idx), fDataModel(idx));
% tmp = corrcoef(fMeanData(idx), fMeanScaled(idx));
% rho = tmp(2);
...
% set(gca, 'Position', pos);
% 
% text(max(fMeanData), min(ylim), ['\rho = ', num2str(rho, 2)],...
%     'HorizontalAlignment', 'Right', 'VerticalAlignment', 'Bottom', 'FontSize', 14, 'Color', 'b')
% text(min(fMeanData), max(ylim), ['\rho = ', num2str(rho2, 2)],...
...
if ~isfield(optIn, 'decisionGroups')
    optOut.decisionGroups = {'LR'};
    optOut.decisionGroupLabels = {'All'};
end
